...
import {ChatMessageHistory} from "@langchain/community/dist/stores/message/in_memory";
...
const getAgent = async () => {
  ...
  const messageWithHistory = new RunnableWithMessageHistory({
    // @ts-ignore
...
async function getMBTIChatChain() {
  ...
  const mbtiChatChain = RunnableSequence.from([
    prompt, model, new StringOutputParser()
  ]);
  return mbtiChatChain;
}
